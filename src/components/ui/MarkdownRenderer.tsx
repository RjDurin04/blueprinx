...
const H1 = ({ node: _node, ...props }: MdProps) => (
    <h1
...
const H2 = ({ node: _node, ...props }: MdProps) => (
    <h2
...
const H3 = ({ node: _node, ...props }: MdProps) => (
    <h3
...
const H4 = ({ node: _node, ...props }: MdProps) => (
    <h4
...
const P = ({ node: _node, ...props }: MdProps) => (
    <p
...
const UL = ({ node: _node, ...props }: MdProps) => (
    <ul className="my-4 ml-5 space-y-1.5 list-none" {...props} />
);

const OL = ({ node: _node, ...props }: MdProps) => (
    <ol
...
const LI = ({ node: _node, ...props }: MdProps) => (
    <li
...
const Blockquote = ({ node: _node, ...props }: MdProps) => (
    <blockquote
...
const Code = ({ node: _node, inline, className, children, ...props }: MdCodeProps) => {
    ...
    return !inline && match ? (
        ...
        <code
            className="text-[13px] font-[family-name:var(--font-jetbrains)] text-[#845A34] px-1 py-0.5"
            {...props}
        >
            {children}
        </code>
...
const Thead = ({ node: _node, ...props }: MdProps) => (
    <thead className="bg-[#F5F3F0]" {...props} />
);

const Th = ({ node: _node, ...props }: MdProps) => (
    <th
...
const Td = ({ node: _node, ...props }: MdProps) => (
    <td
...
const Anchor = ({ node: _node, ...props }: MdProps) => (
    <a
...
const Hr = ({ node: _node, ...props }: MdProps) => (
    <hr className="my-12 border-0 h-px bg-[#E8E4E0]" {...props} />
);

const Strong = ({ node: _node, ...props }: MdProps) => (
