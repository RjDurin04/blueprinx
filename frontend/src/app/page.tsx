...
export default function Home() {
  ...
  const handleGenerate = async () => {
    ...
    // Artificial delay for smoother transition experience
  ...
  return (
    <div className="w-full min-h-[calc(100vh-12rem)] flex flex-col items-center justify-center px-4 sm:px-8 md:px-12 lg:px-16 animate-fadeSlideIn">
      <div className="w-full max-w-[1240px] grid grid-cols-1 lg:grid-cols-[1.2fr_0.8fr] gap-12 lg:gap-24 items-center">
        ...
        <div className="flex flex-col items-start">
          <div className="flex items-center gap-3 mb-6 sm:mb-8">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.3em] text-[#C8956C]">
              Blueprinx v1.0
            </span>
          </div>
          ...
          <div className="hidden lg:flex flex-wrap gap-2 max-w-[500px] opacity-40">
            {['Business Rules', 'User States', 'System Events', 'App Modules', 'Workflows', 'Edge Cases'].map((tag) => (
              <span
                key={tag}
                className="text-[9px] font-black uppercase tracking-[0.2em] text-[#0C0C1D] border border-[#E8E4E0] px-3 py-1.5 bg-white/50 backdrop-blur-sm"
              >
                {tag}
              </span>
            ))}
        ...
        <div className="w-full relative group">
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#F0EDE8] bg-white/50">
              ...
              <span className="text-[9px] font-black uppercase tracking-[0.2em] text-[#A0A0A8]">
                App Details
              </span>
            </div>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center px-6 sm:px-8 py-5 sm:py-6 border-t border-[#F0EDE8] bg-white/50 gap-4">
              <div className="flex items-center gap-3">
                <div className="px-2 py-1 text-[9px] font-black uppercase tracking-[0.1em] text-[#A0A0A8] border border-[#E8E4E0] bg-white shadow-sm">
                  ⌘ Enter
                </div>
                <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-[#C0BCB6]">
                  to start
                </span>
              </div>
          ...
          <div className="absolute top-1/2 -right-8 translate-x-full -translate-y-1/2 hidden xl:block">
            <span className="text-[10px] font-black uppercase tracking-[0.5em] text-[#E8E4E0] vertical-text transform rotate-180" style={{ writingMode: 'vertical-rl' }}>
              Workspace 01
            </span>
          </div>
        </div>
      ...
      <div className="mt-16 lg:hidden flex flex-wrap justify-center gap-2 opacity-40">
        {['Rules', 'States', 'Events', 'Modules'].map((tag) => (
          <span
            key={tag}
            className="text-[8px] font-black uppercase tracking-[0.2em] text-[#0C0C1D] border border-[#E8E4E0] px-2 py-1 bg-[#FAFAF7]"
          >
            {tag}
          </span>
        ))}
